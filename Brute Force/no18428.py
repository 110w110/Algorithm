"""
함수 만들기 notCatch -> 모든 T에서 상하좌우로 움직이면서 S가 있으면 false 리턴
입력 받기
빈 칸 좌표 모으기
빈 칸 좌표 중에 3개 선택하는 조합 배열 만들기
조합의 경우의 수 순회하면서 notCatch()가 true면 true 리턴
끝까지 false만 나오면 false 리턴
"""
    ...
    for t in T:
        ...
        for d in D:
            ...
            while (0 <= dx < N) and (0 <= dy < N):
                if Map[dy][dx] == 'S':
                    return False
                elif Map[dy][dx] == 'W' or Map[dy][dx] == 'T':
                    break
                dx, dy = dx + d[0], dy + d[1]
    return True
...
W = list(combinations(X,3))

result = "NO"
for w in W:
    Map_copied = copy.deepcopy(Map)
    if cannotCatch(Map_copied,w,T,N):
        result = "YES"
print(result)
